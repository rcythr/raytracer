...
/*
 * A no nonsense xml parser for C++11. It's certainly not perfect, but it will parse AT LEAST the valid xml inputs.
 *
 * To use simply implement a handler class with 3 methods:
 *     1. open(std::string& tag_name, std::map<std::string, std::string>& attributes)
            ...
            case ParserState::InOpeningTagName:
            {
                switch(c)
                {
                    ...
                    case '>':
                    {
                        ...
                        if(also_closes) { handler.close(tag_name); }
            ...
            case ParserState::InTagAttribute:
            {
                switch(c)
                {
                    ...
                    case '>':
                    {
                        if(attribute_name != "")
                        {
                            attributes.insert(std::make_pair(attribute_name, std::string("")));
                        }
            ...
            case ParserState::InTagAttributeValue:
            {
                switch(c)
                {
                    ...
                    case ' ':
                    {
                        ...
                        attributes.insert(std::make_pair(attribute_name, attribute_value));
                        attribute_name.clear();
