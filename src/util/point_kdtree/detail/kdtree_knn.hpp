...
template<typename PointTy, size_t K>
void kNearestNeighborRec(kNNConfig<PointTy, K>& cfg, KDNode<PointTy>* node, size_t dim=0) {
    if(node == nullptr)
        return;
    ...
    if(cfg.test[dim] < node->data[dim]) {
        ...
        if(!cfg.bpq_full() || abs(node->data[dim] - cfg.test[dim]) < cfg.worst_dist())
        {
