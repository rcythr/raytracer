...
void PNGTexture::load(std::string filename) {
    ...
    if (!infile) {
        throw std::runtime_error(
            "File supplied for PNG texture loading does not exist.");
    }
    ...
    if (png_sig_cmp(sig, 0, 8)) {
        fclose(infile);
        throw std::runtime_error(
            "File supplied for PNG texture loading is invalid or corrupt.");
    }
    ...
    switch (png_get_channels(png_ptr, info_ptr)) {
        ...
        default:
            throw std::runtime_error(
                "Unsupported number of color channels in PNG image.");
    }
...
glm::vec3 PNGTexture::get_rgb(size_t row, size_t col) {
    ...
    switch (format) {
        case TextureFormat::RGB:
            ...
            return glm::vec3(float(ptr[0]) / 256.0f, float(ptr[1]) / 256.0f,
                             float(ptr[2]) / 256.0f);
...
glm::vec4 PNGTexture::get_rgba(size_t row, size_t col) {
    ...
    switch (format) {
        case TextureFormat::RGB:
            row_bytes = width * 3;
            ptr = data + row_bytes * row + (col * 3);
            return glm::vec4(float(ptr[0]) / 256.0f, float(ptr[1]) / 256.0f,
                             float(ptr[2]) / 256.0f, 1.0f);
        case TextureFormat::RGBA:
            row_bytes = width * 4;
            ptr = data + row_bytes * row + (col * 4);
            return glm::vec4(float(ptr[0]) / 256.0f, float(ptr[1]) / 256.0f,
                             float(ptr[2]) / 256.0f, float(ptr[3]) / 256.0f);
    }
}
