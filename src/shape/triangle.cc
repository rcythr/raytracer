...
inline bool plane_box_overlap(glm::vec3 normal, const glm::vec3 vert, const glm::vec3 maxbox)
{
    glm::vec3 vmin, vmax;
    float v;
...
#define AXISTEST_X01(a, b, fa, fb)             \
    p[0] = a*v0[Y] - b*v0[Z];                    \
    p[2] = a*v2[Y] - b*v2[Z];                    \
        if(p[0]<p[2]) {min=p[0]; max=p[2];} else {min=p[2]; max=p[0];} \
...
#define AXISTEST_X2(a, b, fa, fb)              \
    p[0] = a*v0[Y] - b*v0[Z];                    \
    p[1] = a*v1[Y] - b*v1[Z];                    \
        if(p[0]<p[1]) {min=p[0]; max=p[1];} else {min=p[1]; max=p[0];} \
    rad = fa * box_halfsize[Y] + fb * box_halfsize[Z];   \
    if(min>rad || max<-rad) return 0;
...
#define AXISTEST_Z12(a, b, fa, fb)             \
    p[1] = a*v1[X] - b*v1[Y];                    \
    p[2] = a*v2[X] - b*v2[Y];                    \
        if(p[2]<p[1]) {min=p[2]; max=p[1];} else {min=p[1]; max=p[2];} \
