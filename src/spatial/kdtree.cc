...
void KDTreeSpatialIndex::insert(ShapePtr ptr)
{
...
{
    node = kdtree::create<ShapePtr>(shapes);
}

void KDTreeSpatialIndex::find_closest_hit(Ray& ray, std::function<void(HitResult&)> hit_callback)
{
    kdtree::find_closest_hit<ShapePtr, Ray>(node, ray, [&] (std::vector<ShapePtr> possible_hits) -> bool {
...
void KDTreeSpatialIndex::view_all_objects(std::function<void(ShapePtr&)> functor)
{
    for(auto& s : shapes)
    {
        functor(s);
